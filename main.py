...
options.add_argument('--no-sandbox')  # нужно для запуска в некоторых окружениях
options.add_argument('--disable-dev-shm-usage')  # помогает при нехватке shared memory
options.add_argument('--disable-gpu')  # можно добавить для стабильности
